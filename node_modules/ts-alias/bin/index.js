...
var TsAlias = /** @class */ (function () {
    function TsAlias(options) {
        ...
        if ('aliases' in options) {
            ...
            for (var alias in options.aliases)
                this.list[alias] = __assign(__assign({}, options.aliases[alias]), { pathnames: options.aliases[alias].pathnames.map(function (pathname) { return path_1.default.join(process.cwd(), pathname); }) });
    ...
    TsAlias.prototype.processAliases = function (tsAliases, tsBaseDir) {
        var e_1, _a;
        var list = {};
        for (var match in tsAliases) {
            var destinations = tsAliases[match];
            // Détermine if it must be exact match
            var exact = !match.endsWith('/*');
            if (!exact)
            ...
            list[match] = { exact: exact, pathnames: pathnames };
        }
    ...
    TsAlias.prototype.realpath = function (request, strict) {
        ...
        for (var alias in this.list) {
            var _b = this.list[alias], exact = _b.exact, pathnames = _b.pathnames;
            try {
                for (var pathnames_2 = (e_3 = void 0, __values(pathnames)), pathnames_2_1 = pathnames_2.next(); !pathnames_2_1.done; pathnames_2_1 = pathnames_2.next()) {
                ...
                try {
                    if (pathnames_2_1 && !pathnames_2_1.done && (_a = pathnames_2.return)) _a.call(pathnames_2);
                }
    ...
    TsAlias.prototype.forWebpack = function (modulesPath, nodeExternals) {
        var e_4, _a;
        var _this = this;
        this.options.debug && console.log(LogPrefix, "Generating webpack aliases ...");
        var aliases = {};
        var externalsList = {};
        aliasesIt: for (var match in this.list) {
            var _b = this.list[match], exact = _b.exact, pathnames = _b.pathnames;
            var curAliases = [];
            if (modulesPath === undefined)
                curAliases = pathnames;
            else
                try {
                    for (var pathnames_3 = (e_4 = void 0, __values(pathnames)), pathnames_3_1 = pathnames_3.next(); !pathnames_3_1.done; pathnames_3_1 = pathnames_3.next()) {
                        ...
                        if (pathname.startsWith(modulesPath)) {
                            ...
                            if (nodeExternals === true && pathnames.length === 1) {
                                externalsList[match] = { pathname: pathname, exact: exact };
                                continue aliasesIt;
                            }
                        }
                    ...
                    try {
                        if (pathnames_3_1 && !pathnames_3_1.done && (_a = pathnames_3.return)) _a.call(pathnames_3);
                    }
    ...
    TsAlias.prototype.forModuleAlias = function (enableCache) {
        ...
        var moduleAlias = {};
        var cache = {};
        var _loop_2 = function (alias) {
            var _a = this_2.list[alias], exact = _a.exact, pathnames = _a.pathnames;
            // Create a resolver
            moduleAlias[alias] = function (from, request, requestAlias) {
                ...
                try {
                    ...
                    for (var pathnames_4 = (e_5 = void 0, __values(pathnames)), pathnames_4_1 = pathnames_4.next(); !pathnames_4_1.done; pathnames_4_1 = pathnames_4.next()) {
